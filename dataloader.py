...
def DataLoader(config):
    ...
    reco_mask = np.array([x[i] for x in np.array(f_mc["t/passEventSelection"])])
    mc_gen = np.array(f_mc["tgenBefore/Thrust"]) # generator level without hadronic event selection
    gen_mask = np.array(f_mc["tgenBefore/passEventSelection"])
...
if __name__ == "__main__":
    ...
    data, mc_reco, mc_gen, reco_mask, gen_mask = DataLoader(training_conf)
    print(data.shape, mc_reco.shape, reco_mask.shape, mc_gen.shape, gen_mask.shape)
    print(data[:5], mc_reco[:5], mc_gen[:5], reco_mask[:5], gen_mask[:5])